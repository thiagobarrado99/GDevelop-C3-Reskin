...
Last updated: 2026-09-19 (Phase 4 layout view: Construct context menus in `SceneEditor/index.js` `buildContextMenu`, double-click on empty space inserts an object (`InstancesEditor/index.js` `onBackgroundDoubleClicked`), status bar `Mouse · Layer · Zoom` (`InstancesEditor/StatusBar.js`), properties panels list instance variables before behaviours/effects. Icons: the user decided `C3-Icons/` stays local (in `.git/info/exclude`) and will be copied over GDevelop's default icons later — no icon work until then. Flow clean).
...
3. Phase 3 event sheet (see survey + TODO): done — stepped add-condition/action flow, margin numbers, selection, context/Add menus. Left: per-event `Add…` link, event context menu order, hovered-item description in step 2.
4. Phase 4 layout view: done — context menus, double-click insert, status bar, panel order. Left (TODO): dark surround outside the layout frame, Z-order bar, Align ▸, canvas tooltip "Base layer". Then Phase 5 polish.
