...
**Next:** B11 part 2 (tab strip merged into the top bar row), then parity guide **Phase C** (`docs/c3-reference/parity-guide.md`: C1 start page, C2 picker dialogs sized like windows, C3 empty event sheet placeholder, C4 event sheet toolbar …). Also still open: verify the desktop (Electron) Piskel path for the new blank Sprite frame by hand; `Forma 3D` tile opens GDevelop's asset-pack list instead of a blank 3D shape (TODO); `Adicionar à família` object-row entry (no hook yet); instance `Global ☐` toggle (skipped — objects are global by default).
