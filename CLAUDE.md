...
Last updated: 2026-09-18 (late night: Phase 3 started — the add-condition/action dialog follows Construct's steps (`InstructionEditor/C3ObjectPicker.js` tiles with System first → two-column list → parameters, `useC3Steps` in `InstructionEditorDialog.js`), event rows carry margin numbers (`data-event-number`) and cream selection via the theme CSS, instruction context menu and "Add…" menu in Construct's order (`EnumerateEventsMetadata.js`). Flow clean. Still waiting on step 0 for the icons).
...
0. **Waiting on the user**: whether `C3-Icons/` (Scirra's icon PNGs, untracked at the repo root) may be committed to the public fork or must stay local with look-alikes drawn for git. Do not `git add` it before that answer. Also confirm the object label colour: `#00768e` (sampled from the icons) is ≈ 2:1 contrast on the grey panels, so text currently uses `--c3-object-label-color: #00b1cc` (Construct's lighter object-type cyan) while icons keep `--c3-object-color`; say if you want one shade for both.
