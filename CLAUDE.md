...
Phase C started: C3 (empty sheet placeholder) and C4 (event sheet toolbar = delete · search · settings) done. **Next:** the rest of parity guide **Phase C** (`docs/c3-reference/parity-guide.md`: C1 start page `ABRIR ▾` / `EXPLORAR EXEMPLOS`, C2 picker dialogs sized like windows with search + `Próximo`, C5 instance context menu, C6 layers rows, C7 hide `+`/`⋮`, C8 properties bar on the event sheet, C9 100 % zoom framing, C10 preview wording), and B11 part 2 (tab strip merged into the top bar row — needs a layout rework of `TabsTitlebar` + `Toolbar`, left for a dedicated session). Also still open: verify the desktop (Electron) Piskel path for the new blank Sprite frame by hand; `Forma 3D` tile opens GDevelop's asset-pack list instead of a blank 3D shape (TODO); `Adicionar à família` object-row entry (no hook yet); instance `Global ☐` toggle (skipped — objects are global by default).
