...
**Next:** parity guide **Phase B** (`docs/c3-reference/parity-guide.md`, B1 `Adicionar evento` opens the picker → B2 Add… menu → B3 properties bar …). Also still open: verify the desktop (Electron) Piskel path for the new blank Sprite frame by hand; `Forma 3D` tile opens GDevelop's asset-pack list instead of a blank 3D shape (TODO); status bar reads `Layer: Layer 0` while the layer row says `Camada 0` (untranslated label + base-layer name, `InstancesEditor` status bar).
