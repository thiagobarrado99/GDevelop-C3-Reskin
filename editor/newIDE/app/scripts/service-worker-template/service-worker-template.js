 ...
 */
// c3: the editor may be served under a sub-path (e.g. /editor/): every URL
// the worker handles is relative to its own scope.
const C3_BASE_PATH = new URL(self.registration.scope).pathname.replace(
  /\/$/,
  ''
);
...
self.addEventListener('fetch', event => {
  ...
  if (url.pathname.startsWith(C3_BASE_PATH + '/browser_sw_preview/')) {
    const relativePath = url.pathname.replace(
      C3_BASE_PATH + '/browser_sw_preview',
      ''
    );
...
if (workbox) {
  ...
  /* custom cache rules*/
  workbox.routing.registerNavigationRoute(C3_BASE_PATH + '/index.html', {
    blacklist: [
      new RegExp('^' + C3_BASE_PATH + '/_'),
      /\/[^\/]+\.[^\/]+$/,
      new RegExp('^' + C3_BASE_PATH + '/browser_sw_preview/'),
    ],
  });
