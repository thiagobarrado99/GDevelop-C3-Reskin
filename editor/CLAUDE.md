...
**Folder move (2026-09-19):** this fork now lives in `D:/Projetos/GDevelop-C3-Reskin/editor/` (its own git repo, unchanged remotes); the parent folder is a Laravel 13 app (`https://assemble.theiagod.com`, landing + future login/limits API) whose `public/editor/` receives the editor's web build — see the root `CLAUDE.md` and `deploy/README.md` there. Web builds are made with `PUBLIC_URL=/editor npm run build:editor` (`GameEngineFinder/BrowserS3GDJSFinder.js` serves the fork's GDJS runtime from `<PUBLIC_URL>/GDJS` instead of GDevelop's CDN; `public/.htaccess` = Apache cache headers). Every path in this file is relative to `editor/`.
