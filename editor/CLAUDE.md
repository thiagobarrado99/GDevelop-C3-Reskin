...
**Folder move (2026-09-19):** the fork's files now live in `editor/` of the same git repo (`D:/Projetos/GDevelop-C3-Reskin`, branch `c3-reskin`, `origin` = thiagobarrado99/GDevelop-C3-Reskin, history kept as renames in `fd2ca71e0`); the repo root is a Laravel 13 app (`https://assemble.theiagod.com`, landing + future login/limits API) whose `public/editor/` receives the editor's web build — see the root `CLAUDE.md` and `deploy/README.md`. Web builds: `bash deploy/build-editor.sh` from the root (`PUBLIC_URL=/editor npm run build:editor`; `GameEngineFinder/BrowserS3GDJSFinder.js` serves the fork's GDJS runtime from `<PUBLIC_URL>/GDJS` instead of GDevelop's CDN, the service worker and the preview URLs are scope-relative, `public/.htaccess` = Apache cache headers, theme CSS icon URLs are relative to `static/css/`). Every path in this file is relative to `editor/`; commands too (`cd editor/newIDE/app && npm start`). **Upstream merges now need the subtree prefix** — see "Git workflow".
...
- Clone at `D:/Projetos/GDevelop-C3-Reskin` (blobless partial clone; `git fetch` lazily pulls blobs — first checkout of old files may be slow); the fork's files are under `editor/` since 2026-09-19.
...
- Work on `c3-reskin` branch off `master`. Keep `master` = upstream (files at the repo root, as upstream has them).
- Sync (since the move to `editor/`, 2026-09-19): `git fetch upstream && git checkout master && git merge --ff-only upstream/master && git checkout c3-reskin && git merge -X subtree=editor master` — the `subtree=editor` option maps upstream's root tree onto `editor/`. Last synced: `c6d092110` (2026-09-19, before the move).
- Commits: from the repo root; the pre-commit hook (`editor/.husky/pre-commit`, `core.hooksPath=editor/.husky/_`) runs lint-staged in `editor/newIDE/app`.
