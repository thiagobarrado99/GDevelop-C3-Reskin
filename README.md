> **Assemble3** — this fork reworks the GDevelop editor (everything under `newIDE/`) around the layout / event sheet workflow many game-design classrooms already use, in pt-BR by default: project bar, properties bar and layers arranged the familiar way, the add-condition → object → parameters flow, single-key event sheet shortcuts (E, C, A, B, Q, G, X, I, L, D), behaviours offered as tiles (Solid, Jump-thru, Platform, 8 Direction…), HTML5 export only. The engine, runtime and project format are unchanged GDevelop; projects are saved as `.a3p` (plain GDevelop JSON). See `CLAUDE.md` for the design notes and `TODO.md` for progress. Everything below is GDevelop's original README.
